"""Test suite for Phase 6: Refactored gasket_generator.py with hybrid exact arithmetic.
...
print("=" * 70)
print("Phase 6: Refactored gasket_generator.py Test Suite")
print("=" * 70)
...
if sympy_count > 0:
    ...
    print(f"  ⚠ No SymPy expressions found - this may be expected for shallow depths")

print("  ✓ Irrational preservation test PASSED")

# Test 3: Standard Configuration [-1, 2, 2] Still Works
print("\n[Test 3] Standard configuration [-1, 2, 2] at depth 5")

try:
    curvatures = [Fraction(-1), Fraction(2), Fraction(2)]
    circles = list(generate_apollonian_gasket(curvatures, max_depth=5, stream=False))
...
print("\n[Test 4] Type variety in generated circles")
...
for circle in circles:
    if isinstance(circle.curvature, int):
        type_counts['int'] += 1
    elif isinstance(circle.curvature, Fraction):
        type_counts['Fraction'] += 1
    elif isinstance(circle.curvature, sp.Expr):
        type_counts['SymPy'] += 1

print(f"  Curvature type distribution:")
print(f"    int:      {type_counts['int']}")
print(f"    Fraction: {type_counts['Fraction']}")
print(f"    SymPy:    {type_counts['SymPy']}")

print("  ✓ Type variety test PASSED")

# Test 5: Deduplication Still Works
print("\n[Test 5] Hash-based deduplication with ExactNumber types")
...
# Test 6: Deep Generation (Stress Test)
print("\n[Test 6] Deep generation at depth 6 (stress test)")

try:
    curvatures = [Fraction(-1), Fraction(2), Fraction(2)]
    circles = list(generate_apollonian_gasket(curvatures, max_depth=6, stream=False))

    print(f"  ✓ Generated {len(circles)} circles at depth 6")
    ...
    print("  ✓ Deep generation stress test PASSED")
...
print("  ✓ Type variety: int, Fraction, and SymPy Expr all present")
print("  ✓ Hash-based deduplication works with ExactNumber types")
print("  ✓ Deep generation works without overflow")
