...
│   │   └── endpoints/
│   │       ├── __init__.py
│   │       ├── gaskets.py       # Gasket CRUD endpoints
│   │       ├── circles.py       # Circle query endpoints
│   │       ├── sequences.py     # Sequence detection/retrieval
│   │       ├── export.py        # SVG/JSON/CSV export
│   │       └── websocket.py     # WebSocket streaming endpoint
│   │
│   ├── core/
│   │   ├── __init__.py
│   │   ├── gasket_generator.py  # Core Apollonian gasket algorithm
│   │   ├── descartes.py         # Descartes Circle Theorem implementation
│   │   ├── sequence_detector.py # Sequence pattern detection
│   │   ├── cache_manager.py     # Lookup table and caching logic
│   │   └── circle_math.py       # Circle geometry utilities
│   │
│   ├── db/
│   │   ├── __init__.py
...
│   │   └── models/
│   │       ├── __init__.py
│   │       ├── gasket.py        # Gasket model
│   │       ├── circle.py        # Circle model
│   │       ├── sequence.py      # Sequence model
│   │       └── cache.py         # Cache/lookup table model
│   │
│   ├── schemas/
│   │   ├── __init__.py
│   │   ├── gasket.py            # Pydantic schemas for gasket
│   │   ├── circle.py            # Pydantic schemas for circle
│   │   ├── sequence.py          # Pydantic schemas for sequence
│   │   └── export.py            # Export format schemas
│   │
│   ├── services/
│   │   ├── __init__.py
│   │   ├── gasket_service.py    # Business logic for gasket operations
│   │   ├── sequence_service.py  # Sequence analysis orchestration
│   │   ├── export_service.py    # Export generation (SVG, JSON, CSV)
│   │   └── cache_service.py     # Cache retrieval and storage
│   │
│   ├── utils/
│   │   ├── __init__.py
│   │   ├── rational.py          # Fraction utilities
│   │   ├── hash_utils.py        # Hashing for cache keys
│   │   └── svg_generator.py     # SVG rendering utilities
│   │
│   └── tests/
│       ├── __init__.py
│       ├── conftest.py
│       ├── test_gasket_generator.py
...
│   │   ├── components/
│   │   │   ├── GasketCanvas/
│   │   │   │   ├── GasketCanvas.jsx          # Main Konva canvas
│   │   │   │   ├── CircleLayer.jsx           # Renders circles
│   │   │   │   ├── ControlsOverlay.jsx       # Pan/zoom/reset controls
│   │   │   │   └── useCanvasInteraction.js   # Custom hook for interactions
│   │   │   │
│   │   │   ├── ConfigurationPanel/
│   │   │   │   ├── ConfigurationPanel.jsx    # Initial curvatures input
│   │   │   │   ├── CurvatureInput.jsx        # Individual curvature field
│   │   │   │   ├── DepthSelector.jsx         # Recursion depth slider
│   │   │   │   └── GenerateButton.jsx        # Trigger generation
│   │   │   │
│   │   │   ├── SequencePanel/
│   │   │   │   ├── SequencePanel.jsx         # Sequence list and controls
│   │   │   │   ├── SequenceTypeSelector.jsx  # Dropdown for sequence types
│   │   │   │   ├── SequenceList.jsx          # List of active sequences
│   │   │   │   ├── SequenceItem.jsx          # Single sequence with toggle
│   │   │   │   └── ColorPicker.jsx           # Color selection for highlighting
│   │   │   │
│   │   │   ├── CircleDetails/
│   │   │   │   ├── CircleDetails.jsx         # Expandable tree view
│   │   │   │   ├── CircleInfo.jsx            # Circle properties display
│   │   │   │   ├── TangencyList.jsx          # List of tangent circles
│   │   │   │   └── ParentLinks.jsx           # Parent circle navigation
│   │   │   │
│   │   │   ├── CircleListView/
│   │   │   │   ├── CircleListView.jsx        # Searchable circle list
│   │   │   │   ├── CircleListItem.jsx        # List item with selection
│   │   │   │   └── SearchFilter.jsx          # Filter by properties
│   │   │   │
│   │   │   ├── ExportDialog/
│   │   │   │   ├── ExportDialog.jsx          # Export modal
│   │   │   │   ├── SVGExportOptions.jsx      # SVG-specific options
│   │   │   │   └── DataExportOptions.jsx     # JSON/CSV options
│   │   │   │
│   │   │   └── common/
│   │   │       ├── LoadingSpinner.jsx
│   │   │       ├── ErrorBoundary.jsx
...
│   │   │
│   │   ├── stores/
│   │   │   ├── gasketStore.js       # Gasket configuration and data
│   │   │   ├── sequenceStore.js     # Active sequences and highlighting
│   │   │   ├── selectionStore.js    # Selected circles
│   │   │   ├── uiStore.js           # UI state (loading, errors, panels)
│   │   │   └── canvasStore.js       # Canvas pan/zoom state
│   │   │
│   │   ├── services/
│   │   │   ├── api.js               # Axios instance and interceptors
│   │   │   ├── gasketService.js     # API calls for gaskets
│   │   │   ├── sequenceService.js   # API calls for sequences
│   │   │   ├── exportService.js     # Export API calls
│   │   │   └── websocketService.js  # WebSocket connection management
│   │   │
│   │   ├── hooks/
│   │   │   ├── useGasketGeneration.js  # Gasket generation with WS
│   │   │   ├── useSequenceDetection.js # Sequence detection logic
│   │   │   ├── useCircleSelection.js   # Circle selection sync
│   │   │   └── useExport.js            # Export functionality
│   │   │
│   │   ├── utils/
│   │   │   ├── colorSchemes.js      # Predefined color schemes
│   │   │   ├── circleGeometry.js    # Client-side geometry utils
...
```
App
├── Layout (MUI Container/Grid)
│   ├── Header
│   │   ├── AppBar with title
│   │   └── Configuration quick access
│   │
│   ├── MainContent (Grid container)
│   │   ├── LeftPanel (Grid item, 30%)
│   │   │   ├── ConfigurationPanel
│   │   │   │   ├── CurvatureInput (x3 or x4)
│   │   │   │   ├── DepthSelector
│   │   │   │   └── GenerateButton
│   │   │   │
│   │   │   ├── SequencePanel
...
│   │   │       ├── SearchFilter
│   │   │       └── VirtualizedList
│   │   │           └── CircleListItem (repeating)
│   │   │
│   │   ├── CenterPanel (Grid item, 50%)
│   │   │   └── GasketCanvas
│   │   │       ├── Stage (Konva)
│   │   │       │   └── Layer
│   │   │       │       └── Circle (repeating, Konva.Circle)
│   │   │       │
│   │   │       └── ControlsOverlay
│   │   │           ├── ZoomIn/Out buttons
│   │   │           ├── ResetView button
│   │   │           └── LoadingIndicator
│   │   │
│   │   └── RightPanel (Grid item, 20%)
│   │       ├── CircleDetails
│   │       │   ├── Accordion (Sequence Info)
│   │       │   ├── Accordion (Circle Properties)
│   │       │   │   ├── Curvature display
│   │       │   │   ├── Center coordinates
│   │       │   │   ├── Radius
│   │       │   │   └── Generation
...
│   │       │   │   └── ParentLinks (clickable)
│   │       │   │
│   │       │   └── Accordion (Tangencies)
│   │       │       └── TangencyList (clickable)
│   │       │
│   │       └── ExportButton
│   │           → Opens ExportDialog
│   │
│   └── Footer
│       └── Status bar (generation stats, cache hits)
│
└── Dialogs
    ├── ExportDialog
    │   ├── TabPanel (SVG)
...
**Formula**:
Given three circles with curvatures k₁, k₂, k₃, the curvature k₄ of the two circles tangent to all three:
...
Given centers as complex numbers z₁, z₂, z₃ with curvatures k₁, k₂, k₃:
...
Circles with curvatures ≡ r (mod 24).
...
### 8.4 Hybrid Exact Arithmetic System

#### 8.4.1 Overview and Rationale

**Problem Statement**:

The original implementation used `fractions.Fraction` with `.limit_denominator()` for all calculations. This caused two critical issues:

1. **INTEGER Overflow**: When solving tangency constraints symbolically, irrational solutions (containing √2, √3, etc.) were approximated as Fractions with denominators up to 10⁹. Through repeated Descartes theorem applications, denominators grew exponentially, exceeding SQLite's 63-bit INTEGER limit at depth 2-3.

2. **Loss of Exactness**: The `.limit_denominator()` approximation violated the design principle of "exact arithmetic throughout."

**Example Failure**:
```python
# Input curvatures: [1, 2, 2]
# Solving for third circle position returns:
#   x = 7/6
#   y = 2√2/3  ← IRRATIONAL!

# Old approach:
y_float = float((2 * sqrt(2) / 3).evalf(50))  # 0.942809...
y_frac = Fraction(y_float).limit_denominator(10**9)
# Result: Fraction(581953685, 617406144)  ← HUGE denominator!

# After 2 Descartes iterations:
# Denominator grows to ~10¹⁸ → exceeds 63-bit limit → DATABASE ERROR
```

**Solution**:

Implement an **intelligent hybrid system** that automatically selects the most efficient exact representation based on the mathematical nature of each result:

- **int**: For integer results (fastest, 50x speedup over SymPy)
- **Fraction**: For rational results (medium speed, exact)
- **SymPy Expr**: For irrational results (preserves √2, √3, etc. symbolically)

**Benefits**:

1. ✅ **True Exactness**: Irrational numbers preserved symbolically (no approximation)
2. ✅ **No Overflow**: SymPy expressions have no bit-length limits
3. ✅ **Performance**: 15-25x speedup for integer-heavy configurations
4. ✅ **Simplicity**: Transparent to calling code via utility functions
...
ExactComplex = Union[
    ...
    sp.Expr,  # SymPy complex expression (real + imag*I)
]
```
...
```
Input: Result from mathematical operation

1. Simplify result (if SymPy expression)
   ↓
2. Check: Is it exactly an integer?
   YES → Return Python int
   NO  → Continue
   ↓
3. Check: Is it rational (no sqrt, no irrationals)?
   YES → Return Fraction(numerator, denominator)
   NO  → Continue
   ↓
4. Must be irrational → Return SymPy Expr
```

**Performance Characteristics**:

| Type | Arithmetic Speed | Use Case |
|------|-----------------|----------|
| **int** | ⚡ Fastest (native) | Integer curvatures, perfect square discriminants |
| **Fraction** | 🔥 Fast (Python) | Rational curvatures, exact divisions |
| **SymPy Expr** | 🐢 Slower (symbolic) | Irrational coordinates, algebraic expressions |

**Expected Gasket Generation Speedup**:

| Configuration | Dominant Type | Speedup vs Pure SymPy |
|--------------|---------------|----------------------|
| [-1, 2, 2] (enclosing) | 70% int, 30% Fraction | **25-30x** |
| [2, 3, 6] (rational) | 40% int, 60% Fraction | **15-20x** |
| [1, 2, 2] (irrational) | 10% int, 20% Fraction, 70% SymPy | **5-10x** |

#### 8.4.3 Core Module - exact_math.py

**File**: `backend/core/exact_math.py`

**Key Functions**:

- **Type Detection**: `is_sympy_integer()`, `is_sympy_rational()`, `sympy_to_exact()`
- **Arithmetic**: `smart_add()`, `smart_multiply()`, `smart_divide()`, `smart_sqrt()`
- **Conversions**: `to_string()`, `format_exact()`, `parse_exact()`, `to_sympy()`
- **Complex Ops**: `smart_complex_multiply()`, `smart_complex_sqrt()`

**Example Usage**:
```python
from core.exact_math import smart_add, smart_multiply, smart_sqrt
...
# Irrational arithmetic (symbolic)
>>> smart_sqrt(2)
sqrt(2)  # SymPy Expr

# Automatic type detection in Descartes theorem
>>> k1, k2, k3 = -1, 2, 2
>>> discriminant = k1*k2 + k2*k3 + k3*k1  # = 0
>>> sqrt_disc = smart_sqrt(discriminant)
>>> type(sqrt_disc), sqrt_disc
(<class 'int'>, 0)  # Detected perfect square!
```

#### 8.4.4 Database Storage Format
...
|------|--------|---------|
| **int** | `int:<value>` | `"int:6"` |
| **Fraction** | `frac:<num>/<denom>` | `"frac:3/2"` |
| **SymPy** | `sym:<sympy_str>` | `"sym:sqrt(2)"` |

**Schema Changes**:

```sql
-- New columns (primary, exact storage)
ALTER TABLE circles ADD COLUMN curvature_exact TEXT NOT NULL;
ALTER TABLE circles ADD COLUMN center_x_exact TEXT NOT NULL;
ALTER TABLE circles ADD COLUMN center_y_exact TEXT NOT NULL;
ALTER TABLE circles ADD COLUMN radius_exact TEXT NOT NULL;

-- Legacy columns kept for backwards compatibility
-- (curvature_num, curvature_denom, etc. remain)
```

**Example Database Rows**:

```sql
-- Integer-heavy gasket ([-1, 2, 2])
id | curvature_exact | center_x_exact | center_y_exact | ...
1  | int:3           | int:0          | int:0          | ...
2  | int:6           | frac:1/2       | int:0          | ...

-- Irrational gasket ([1, 2, 2])
id | curvature_exact     | center_x_exact | center_y_exact    | ...
3  | sym:5 + 4*sqrt(2)   | frac:7/6       | sym:2*sqrt(2)/3   | ...
```

#### 8.4.5 Migration Strategy

**Alembic Migration**: Add new TEXT columns alongside existing INTEGER columns for backwards compatibility.

**Storage Strategy**:
```python
# When persisting:
circle = Circle(
    curvature_exact=format_exact(circle_data.curvature),  # Primary
    curvature_num=to_numerator_denominator(...)[0],       # Legacy (lossy)
    curvature_denom=to_numerator_denominator(...)[1]
)

# When retrieving:
@hybrid_property
def curvature(self) -> ExactNumber:
    if self.curvature_exact:
        return parse_exact(self.curvature_exact)  # Exact
    else:
        return Fraction(self.curvature_num, self.curvature_denom)  # Fallback
```

#### 8.4.6 Testing Strategy

**Unit Tests** (`test_exact_math.py`): 80+ tests covering:
- Type detection (20 tests)
- Arithmetic operations (30 tests)
- Conversions (20 tests)
- Complex operations (10 tests)

**Integration Tests** (`test_hybrid_integration.py`):
- End-to-end gasket generation with irrational configs
- Database round-trip with all three types
- API integration (verify `[1, 2, 2]` no longer causes overflow)

**Performance Tests**:
- Benchmark depth-10 gasket: target 15-25x speedup
- Measure type distribution (verify int/Fraction used when possible)

**Reference**: See `DESIGN_SPEC.md` Section 8.5 for complete specifications.
...
1. Build `CircleDetails` component
   ...
   - Parent links (clickable → select parent)
   - Tangency list (clickable → select tangent)
...
**Testing**:
- Click circle on canvas → details update
- Click circle in list → canvas highlights
...
**Why**:
- Apollonian gaskets are deterministic: same initial curvatures → same gasket
...
- Full user workflows (generate → detect → export)
...
- Clear interfaces between layers (API ↔ Service ↔ Core)
