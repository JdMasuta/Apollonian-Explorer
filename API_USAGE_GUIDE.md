> **⚠️ HISTORICAL DOCUMENT (pre-revamp).** Large parts of this specification
> describe the retired architecture (hybrid SymPy arithmetic, dual INTEGER/TEXT
> storage, triplet-BFS generation). The current system is the exact
> inversive-coordinate engine with schema v2, resolution-driven generation,
> word-replay deepening, parabolic cusp chains and Möbius transforms.
> **Authoritative references:** `REVAMP_BLUEPRINT.md` (architecture),
> `HISTORY.md` (what was built, when, and why), the endpoint docstrings under
> `backend/api/endpoints/`, and `backend/core/engine/` module docstrings.
